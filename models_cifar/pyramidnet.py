    ...
    def forward(self, x: torch.Tensor):
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(
                x.new_zeros(batch_size, residual_channel - shortcut_channel, featuremap_size[0], featuremap_size[1]))
    ...
    def forward(self, x, boxes=None, share_fc=False):
        if self.dataset == 'cifar10' or self.dataset == 'cifar100':
            ...
            if boxes is not None:
                ...
                roi_feat = roi_align(feat_map,
                                     boxes,
                                     output_size=(1, 1),
                                     spatial_scale=spatial_scale,
                                     sampling_ratio=-1,
                                     aligned=True).squeeze()
                if share_fc:
...
def pyramidnet_200_240(num_classes=100):
    net = PyramidNet(dataset='cifar100', depth=200, alpha=240, num_classes=num_classes, bottleneck=True)
    return net


def pyramidnet_164_270(num_classes=100):
    net = PyramidNet(dataset='cifar100', depth=164, alpha=270, num_classes=num_classes, bottleneck=True)
    return net


if __name__ == '__main__':
    net = pyramidnet_200_240(100)
    img = torch.rand(10, 3, 32, 32)
    boxes = torch.Tensor([0, 0, 0, 0]).float()
    y = net(img, None)
    print(len(y))