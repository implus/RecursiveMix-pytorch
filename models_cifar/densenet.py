...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, growth_rate):
        ...
        #as DenseNet-B."""
        self.bottle_neck = nn.Sequential(nn.BatchNorm2d(in_channels), nn.ReLU(inplace=True),
...
class Transition(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        #layer""".
        self.down_sample = nn.Sequential(nn.BatchNorm2d(in_channels), nn.Conv2d(in_channels,
                                                                                out_channels,
                                                                                1,
                                                                                bias=False), nn.AvgPool2d(2, stride=2))
...
class DenseNet(nn.Module):
    def __init__(self, block, nblocks, growth_rate=12, reduction=0.5, num_classes=100):
        ...
        self.linear_roi = nn.Linear(inner_channels, num_classes)
    ...
    def forward(self, x, boxes=None, share_fc=False):
        ...
        if boxes is not None:
            ...
            if share_fc:
                out_roi = self.linear(roi_feat)
            else:
                out_roi = self.linear_roi(roi_feat)
